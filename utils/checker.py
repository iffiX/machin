...
def _gen_check_hook(writer, name, counter, dir, path="tmp/log/", param_freq=100, check_input=True, check_param=True):
        ...
        with torch.no_grad():
            ...
            for i in range(len(args)):
                if check_input and isinstance(input[i], (torch.Tensor, np.ndarray)):
                    check_nan(input[i], name + ".{}.arg.{}_{}".format(dir, args[i], counter.get()))
                    #if dir == "backward":
                    #    torch.save(input[i], path + name + ".{}.{}_{}".format(dir, args[i], counter.get()))
                    writer.add_scalars(name + ".{}.arg.{}".format(dir, args[i]),
...
def check_model(writer, model, global_counter,
