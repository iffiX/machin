...
class TestDDPPlugin:
    def test_all(self, tmpdir):
        ...
        process_0 = sp.Popen([sys.executable,
                              p.join(p.dirname(p.abspath(__file__)),
                                     "_pl_plugin_runner.py"),
                              "ddp"],
                             env=env)
        ...
        if process_0.poll():
            ...
            pytest.fail("DDPPlugin script returned code {}."
                        .format(process_0.returncode))
        else:
...
class TestDDPSpawnPlugin:
    def test_all(self, tmpdir):
        ...
        if process_0.poll():
            process_0.kill()
            pytest.fail("Timeout on waiting for the DDPSpawnPlugin "
                        "script to end.")
        elif process_0.returncode != 0:
            pytest.fail("DDPSpawnPlugin script returned code {}."
                        .format(process_0.returncode))
        else:
            with open(test_save_path, "rb") as f:
                flags = pickle.load(f)
