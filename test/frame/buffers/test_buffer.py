...
class TestBuffer(object):
    ...
    param_test_append = [
        ...
        ([{"state": {"state_1": t.zeros([1, 2])},
         ...
         ("state", "action", "next_state", "reward", "terminal"),
         ValueError, "Transition object has different attributes"),
    ]
    ...
    def const_buffer(self, pytestconfig):
        ...
        buffer = Buffer(buffer_size=self.SAMPLE_BUFFER_SIZE,
    ...
    @pytest.mark.parametrize("batch_size", [
        0, int(SAMPLE_BUFFER_SIZE / 2), SAMPLE_BUFFER_SIZE,
        SAMPLE_BUFFER_SIZE * 2
    ])
    @pytest.mark.parametrize("concat", [True, False])
    @pytest.mark.parametrize("dev", [None, "cpu"])  # buffer already on gpu
    @pytest.mark.parametrize("sample_method", [
        "random", "random_unique", "all", "some_invalid_method"
    ])
    @pytest.mark.parametrize("sample_attrs,concat_attrs,should_be_attrs", [
        # Case 0
        (None,
         None,
         ["state", "action", "next_state", "reward", "terminal",
          "data_index", "not_concatenable"]),
    ...
    def test_sample(self, const_buffer, batch_size, concat, dev,
        ...
        if sample_method == "some_invalid_method":
            with pytest.raises(RuntimeError,
                               match="Cannot find specified sample method"):
            ...
            if bsize == 0:
                ...
                for data, attr in zip(b, should_be_attrs):
                    if attr == "state":
                        if concat:
                            assert self.t_eq(data["state_1"],
                                             t.zeros([bsize, 2]))
                        else:
                            assert (isinstance(data["state_1"], list) and
                                    len(data["state_1"]) == bsize and
                                    self.t_eq(data["state_1"][0],
                                              t.zeros([1, 2])))
                    elif attr == "action":
                        if concat:
                            assert self.t_eq(data["action_1"],
                                             t.zeros([bsize, 3]))
                        else:
                            assert (isinstance(data["action_1"], list) and
                                    len(data["action_1"]) == bsize and
                                    self.t_eq(data["action_1"][0],
                                              t.zeros([1, 3])))
                    elif attr == "next_state":
                        if concat:
                            assert self.t_eq(data["next_state_1"],
                                             t.zeros([bsize, 4]))
                        else:
                            assert (isinstance(data["next_state_1"], list) and
                                    len(data["next_state_1"]) == bsize and
                                    self.t_eq(data["next_state_1"][0],
                                              t.zeros([1, 4])))
                    elif attr == "reward":
                        if concat:
