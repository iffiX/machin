...
class TestSaveEnv:
    def test_all(self, tmpdir):
        ...
        save_env = SaveEnv(env_root=tmp_dir,
                           restart_from_trial=os.path.basename(
                               save_env.get_trial_root()))

        # check directories
        t_root = save_env.get_trial_root()
        assert (os.path.exists(join(t_root, "model")) and
                os.path.isdir(join(t_root, "model")) and
                os.path.exists(join(t_root, "config")) and
                os.path.isdir(join(t_root, "config")) and
                os.path.exists(join(t_root, "log", "images")) and
                os.path.isdir(join(t_root, "log", "images")) and
                os.path.exists(join(t_root, "log", "train_log")) and
                os.path.isdir(join(t_root, "log", "train_log")))

        save_env.create_dirs(["some_custom_dir"])
        assert (os.path.exists(join(t_root, "some_custom_dir")) and
        ...
        assert (save_env.get_trial_image_dir() ==
                join(t_root, "log", "images"))
        assert (save_env.get_trial_train_log_dir() ==
                join(t_root, "log", "train_log"))
